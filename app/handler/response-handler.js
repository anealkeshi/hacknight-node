...
class ResponseHandler {
    ...
    static  getDefaultResponseHandler(res) {
        return {
            onSuccess: function (data, message, code) {
                ResponseHandler.respondWithSuccess(res, code || ResponseHandler.HTTP_STATUS.OK, data, message);
            },
            onError: function (error) {
                ResponseHandler.respondWithError(res, error.status || 500, error.message || 'Unknown error');
            }
        };
    }
    ...
    static  getDefaultResponseHandlerData(res) {
        return {
            ...
            onError: function (error) {
                ResponseHandler.respondWithErrorData(res, error.status, error.message, error.data);
            }
        };
    }
